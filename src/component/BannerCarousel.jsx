...
const BannerCarousel = () => {
    ...
    const handleNavigate = () => {
        if (selectedCategory) {
            navigate(`/listing?category=${selectedCategory}`);
        }
    };
            ...
            <div className="hidden md:block absolute font-lora bottom-10 right-10 z-10 bg-red-500 p-6 rounded-2xl shadow-2xl w-72">
                <div>
                    ...
                    <div className="space-y-4">
                        <select
                            className="w-full p-2 border border-white bg-white text-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400"
                            value={selectedCategory}
                            onChange={(e) => setSelectedCategory(e.target.value)}
                        >
                            <option value="">Chọn một loại hoa</option>
                            <option value="Hoa Bó">Hoa bó</option>
                            <option value="Hoa Thả Bình">Hoa thả bình</option>
                        </select>
                        <button
                            className="w-full bg-yellow-400 text-white font-bold py-2 rounded-lg hover:bg-yellow-500 transition-all duration-300 shadow-md"
            ...
            <div className="p-1.5 mt-[-15px]">
                <div className="md:hidden font-lora inset-0 flex items-center justify-center rounded-sm bg-red-600 p-6 z-20">
                    <div className="w-full max-w-md text-start">
                        ...
                        <div className="space-y-4">
                            <select
                                className="w-full p-3 border border-white bg-white text-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400"
                                value={selectedCategory}
                                onChange={(e) => setSelectedCategory(e.target.value)}
                            >
                                <option value="">Chọn một loại hoa</option>
                            ...
                            <button
                                className="w-full bg-yellow-400 text-white font-bold py-3 rounded-lg hover:bg-yellow-500 transition-all duration-300 shadow-md"
                                onClick={handleNavigate}
                                disabled={!selectedCategory}
                            >
                                Xem ngay
